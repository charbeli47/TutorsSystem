...
class Tutor extends MY_Controller 
{
	...
	function personal_info()
	{
		...
		if(isset($_POST['submitbutt']))
		{
			...
			$this->form_validation->set_rules('bank_ac_details', 'Iban', 'trim|required|xss_clean');
			$this->form_validation->set_rules('Swift', 'Swift', 'trim|required|xss_clean');
...
//end send emaill to student
	...
	function join_link( $primary_key , $row ) {
		...
		$query = 'SELECT b.*, course.name course_name FROM ' . TBL_BOOKINGS . ' b 
			...
			'WHERE (b.status = "session_initiated" OR b.status="running") AND "'.$now.'" BETWEEN start_date AND end_date AND b.tutor_id = ' . $user_id .' AND booking_id = ' . $primary_key;
		// echo $query;
