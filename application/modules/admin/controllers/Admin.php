...
class Admin extends MY_Controller 
{
	...
	{
		return date('d/m/Y', strtotime($value)) . ' ( '.$row->user_name . ' - ' . $row->user_type.' )';
	}
	...
	function addeditseosettings( $id = '' ) {
		if( ! $this->ion_auth->logged_in() || ! $this->ion_auth->is_admin() ){
			safe_redirect( get_safe_url('subscription') );
		}
		$this->data['message'] = $this->session->flashdata('message');
		if (isset($_POST) && !empty($_POST))
		{
			$this->form_validation->set_rules('type', get_languageword('type'), 'trim|required');
			if ($this->form_validation->run() === TRUE) {
				$data = array(
				...
				if( $this->input->post('id') == '' ) {
					...
					$this->base_model->insert_operation( $data, 'seosettings');
					$msg = get_languageword('Record insered successfully');
				} else {
					$data['updated'] = date('Y-m-d H:i:s');
					$this->base_model->update_operation( $data, 'seosettings', array('id' => $this->input->post('id')));
					$msg = get_languageword('Record updated successfully');
				}
				$this->prepare_flashmessage(get_languageword('record updated successfully'), 0);
				redirect('admin/seosettings');	
			}
		}
		
		$this->data['id'] = $id;
		$this->data['activemenu'] 	 = "seosettings";
		$this->data['activesubmenu'] = "seosettings";
		$this->data['content'] 		 = 'addeditseosettings';
		$this->data['pagetitle'] 	 = get_languageword('SEO') . ' ' . get_languageword('settings');
		$this->_render_page('template/admin/admin-template', $this->data);
	}
	...
	function seosettings() {
		...
		$crud->set_table($this->db->dbprefix('seosettings'));

		$crud->set_subject(get_languageword('Setting'));
		$crud->columns('type', 'seo_title','seo_description', 'seo_keywords', 'created');
		$crud->add_fields(array('type', 'seo_title','seo_description', 'seo_keywords'));
		$crud->edit_fields(array('type', 'seo_title','seo_description', 'seo_keywords'));
		$crud->required_fields(array('type', 'seo_title'));
		$crud->unique_fields('type');
		
		$element = array(
			'' => get_languageword( 'Please select' ),
			// 'location'	=>	get_languageword( 'location' ),
			'login'	=>	get_languageword( 'Login Page' ),
			'dynamicpage' => get_languageword( 'Dynamic Page' ),
			'homepage' => get_languageword( 'Home Page' ),
			'about_us' => get_languageword( 'About Us Page' ),
			'faqs' => get_languageword( 'FAQ Page' ),
			'courses_single'	=>	get_languageword( 'Single Course' ),
			'courses'	=>	get_languageword( 'Courses' ),
			'findtutor'=> get_languageword( 'Find Tutor' ),
			'findinstitute'=> get_languageword( 'Find Institute' ),
			'tutor_single' => get_languageword( 'Single Tutor' ),
			'institute_single' => get_languageword( 'Single Institute' ),			
			'student_single'	=>	get_languageword( 'Single Student' ),			
			// 'findleads'=> get_languageword( 'Find Leads' ),			
			// 'categories'=> get_languageword( 'Categories' ),
			// 'courses'=> get_languageword( 'Courses' ),				
		);
		$crud->field_type('type', 'dropdown', $element);
		
		$crud->unset_texteditor(array('seo_title','seo_description', 'seo_keywords'));
