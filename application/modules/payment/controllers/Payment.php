	...
	function insert_transaction( $payment_type, $params = array() ) {
			...
			if ( ! empty( $user_details ) && ! empty( $gateway_details ) && ! empty( $package_details ) ) {
				...
				if ( $payment_type == 'manual' ) {
					...
					$subscription_info['transaction_no']   	= rand();
			...
			if ( isset( $params['is_redirect'] ) && $params['is_redirect'] == 'yes' ) {
				...
				$url = URL_TUTOR_SUBSCRIPTIONS;
				if ( $this->ion_auth->is_student() ) {
					$url = URL_STUDENT_SUBSCRIPTIONS;
				} elseif( $this->ion_auth->is_institute() ) {
					$url = URL_INSTITUTE_SUBSCRIPTIONS;
				} elseif( $this->ion_auth->is_admin() ) {
					$url = URL_ADMIN_STUDENT_BOOKINGS;
				}
				safe_redirect( $url );
			}
