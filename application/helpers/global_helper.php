...
if ( ! function_exists('get_tutor_video'))
{
	function get_tutor_video($video = "", $gender = "male")
	{
		if(!empty($video) && file_exists(URL_UPLOADS_PROFILES_PHYSICAL.$video)) {

			return URL_UPLOADS_PROFILES.$video;
...
if ( ! function_exists( 'safe_redirect')) {
	function safe_redirect( $url = '', $message = '', $message_type = 'info' ) {
		...
		if( $message != '' ) {
			...
			$CI->prepare_flashmessage( get_languageword( $message ), $message_type );
		}
		$new_url = '';
		if( ! $CI->ion_auth->logged_in() ) {
			$new_url = 'auth/login';
			...
			$new_url = 'student/index';
		} elseif( $CI->ion_auth->is_institute() ) {
			$new_url = 'institute/index';
...
function get_safe_url( $case = 'subscription' ) {
	$CI	=&	get_instance();
	$new_url = '';
	if( $case == 'subscription' ) {
		if( ! $CI->ion_auth->logged_in() ) {
			...
			$new_url = URL_TUTOR_SUBSCRIPTIONS;
		} elseif( $CI->ion_auth->is_student() ) {
			$new_url = URL_STUDENT_SUBSCRIPTIONS;
		} elseif( $CI->ion_auth->is_institute() ) {
			$new_url = URL_INSTITUTE_SUBSCRIPTIONS;
		} elseif( $CI->ion_auth->is_admin() ) {
			$new_url = 'admin/payments';
		}
	} else {
		if( ! $CI->ion_auth->logged_in() ) {
			$new_url = 'auth/login';
...
function is_enable_bbb() {
	$CI	=&	get_instance();
	$field_values = $CI->db->get_where('system_settings_fields',array('type_id' => BIGBLUEBUTTON))->result();
	$bbb_enable = false;
	if( ! empty( $field_values ) ) {
		foreach($field_values as $value) {
			if( $value->field_key == 'bbb_enable' && $value->field_output_value == 'yes' ) {
				$bbb_enable = true;
			}
		}
	}
...
function replace_constants( $variables, $message ) {
	$matches = $defaults_existed = array();    
	if(preg_match_all('/\__(.*?)\__/',$message,$match)) {            
			$matches[] = $match[1];            
	}
	if( ! empty( $matches ) ) {
		foreach( $matches[0] as $key => $val ) {
			$defaults_existed[] = '__' . $val . '__';
		}
	}
	...
	if ( is_array( $variables ) && ! empty( $variables ) ) {
		foreach( $variables as $key => $val ) {
			...
			$string = ucwords( str_replace( '-', ' ', $string ) ); // Replaces all '-' with space and make word as camle case.			
			$message = str_replace($key, $string, $message);
		}
		
		// Let us replace all existed constants and not pass any value
		$defaults = constant_defaults();
		if( ! empty( $defaults_existed ) ) {
			foreach( $defaults_existed as $key1 => $val1 ) {
				if( in_array( $val1, array_keys( $defaults ) ) ) {						
					$message = str_replace($val1, '', $message );
				}
			}
		}
	} else {
		$defaults = constant_defaults();
		foreach( $defaults as $key => $val ) { // Let us replace all variables if they used and not send any data
...
function get_seo( $type = '', $variables = array() ) {
	$CI	=&	get_instance();
	$return = array();
	if( $type != '' ) {
		$row = $CI->db->get_where( 'seosettings', array( 'type' => $type ) )->row();
		
		if( $row ) {
			$message = $row->seo_title;
			$return['seo_title'] = replace_constants( $variables, $message);
			
			$message = $row->seo_description;
			$return['seo_description'] = replace_constants( $variables, $message);
			
			$message = $row->seo_keywords;
			$return['seo_keywords'] = replace_constants( $variables, $message);	
		}
	}
	return $return;
}

function constant_defaults() {
	$CI	=&	get_instance();
	return array(
				'__SITE_TITLE__' => '',
...
function tutor_get_config( $item )
{
	$CI	=&	get_instance();
	$item_val = $CI->config->item( $item );
	return ( ! empty( $item_val ) ) ? $item_val : '';
}
?>